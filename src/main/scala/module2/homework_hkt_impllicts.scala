...
object homework_hkt_impllicts{
    ...
    /**
      ...
      * 
      */
    def tupleF[F[_], A, B](fa: Bindable[F, A], fb: Bindable[F, B]): F[(A, B)] = fa.flatMap(a => fb.map(b => (a, b)))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val r1 = println(tupleF(optBindable(optA), optBindable(optB)))
    val r2 = println(tupleF(listBindable(list1), listBindable(list2)))
  }
}